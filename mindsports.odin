...
move_from_mindsports_parse :: proc(str: string) -> (ret: Move, ok: bool) #optional_ok {
	...
	tile: Tile
        ...
        for len := 2; len >= 0; len -= 1 {
                ...
                if id, ok := strconv.parse_uint(str[c:][:len]); ok {
                        c += len
                        tile = tile_from_mindsports_id(u8(id), player)
                        break
                }
        }
...
tile_from_mindsports_id :: proc(id: u8, player: Player) -> (ret: Tile) {
...
test_parse_mindsports :: proc(t: ^testing.T) {
        output := move_from_mindsports_parse("W[P63i9]")
